  ...
  useEffect(() => {
    const handleKey = (e) => {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        e.preventDefault()
        document.getElementById('admin-spotlight')?.focus()
      }
    }
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        id="admin-spotlight"
        type="search"
        placeholder="Cerca partner, lead, portafoglio, transazioni…"
...
function NotificationBell({ open, onToggle, onClose }) {
      ...
      {open && (
        <div className={`absolute right-0 top-full z-50 mt-2 w-80 ${adminGlassCard} py-1 shadow-2xl`}>
          <div className="border-b border-white/10 px-4 py-3">
            ...
            <p className="text-xs text-zinc-500">{unreadCount} in attesa di approvazione</p>
          </div>

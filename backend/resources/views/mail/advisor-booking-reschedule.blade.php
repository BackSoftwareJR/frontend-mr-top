...
<head>
    ...
    <title>Nuova data prenotazione</title>
</head>
<body style="font-family: system-ui, sans-serif; line-height: 1.5; color: #1a1a1a;">
    <p>Ciao {{ $clientName }},</p>
    <p>La tua chiamata gratuita con {{ $advisorName }} è stata spostata a una nuova data.</p>
    <p>
        <strong>Nuova data:</strong> {{ $scheduledDate }}<br>
        <strong>Nuovo orario:</strong> {{ $scheduledTime }}
    </p>
    @if(!empty($leadTitle))
